...
@section('title', 'Edit Device - Easy Ride')
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">Edit Device Information</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('devices.update', $device) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number"
                               ...
                               step="0.01"
                               min="0"
                               value="{{ old('single_price_per_hour', $device->single_price_per_hour) }}" required>
                    ...
                    <div class="mb-3">
                        <div class="form-check">
                            <input class="form-check-input"
                                   ...
                                   value="1"
                                   {{ old('supports_multiplayer', $device->supports_multiplayer) ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('device_type') is-invalid @enderror"
                            ...
                            <option value="PlayStation" {{ old('device_type', $device->device_type) == 'PlayStation' ? 'selected' : '' }}>PlayStation</option>
                            <option value="Room" {{ old('device_type', $device->device_type) == 'Room' ? 'selected' : '' }}>Room</option>
                            <option value="Computer" {{ old('device_type', $device->device_type) == 'Computer' ? 'selected' : '' }}>Computer</option>
                            <option value="Ping Pong" {{ old('device_type', $device->device_type) == 'Ping Pong' ? 'selected' : '' }}>Ping Pong</option>
                            <option value="Billiards" {{ old('device_type', $device->device_type) == 'Billiards' ? 'selected' : '' }}>Billiards</option>
                        </select>
                        @error('device_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('device_status') is-invalid @enderror"
                            ...
                            <option value="Working" {{ old('device_status', $device->device_status) == 'Working' ? 'selected' : '' }}>Working</option>
                            <option value="Maintenance" {{ old('device_status', $device->device_status) == 'Maintenance' ? 'selected' : '' }}>Maintenance</option>
                            <option value="Stopped" {{ old('device_status', $device->device_status) == 'Stopped' ? 'selected' : '' }}>Stopped</option>
                        </select>
                        @error('device_status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
